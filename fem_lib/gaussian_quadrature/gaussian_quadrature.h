...
class OneDimentionSolver {
   public:
    ...
    /// </summary>
    double computeWithRange(double from, double to) const {
        const double coefs[] = {
            (to - from) / 2.0,
            (to + from) / 2.0,
        };
    ...
    double compute() const { return computeWithRange(from, to); }

    /// <summary>
    /// Function to compute quadrature with other range with step. Doesn't override previous ranges
    /// </summary>
    double computeWithStep(double from, double step) const { return computeWithRange(from, from + step); }
};
...
class TwoDimentionalSolver {
   public:
    ...
    double computeWithRange(double xFrom, double xTo, double yFrom, double yTo) const {
    ...
    /// </summary>
    /// <returns>Result of computation</returns>
    double compute() const { return computeWithRange(xFrom, xTo, yFrom, yTo); }

    /// <summary>
    /// Function that return computed integral with fixed range, doesn't change in-object range
    /// </summary>
    /// <returns>Result of computation</returns>
    double computeWithStep(double xFrom, double xStep, double yFrom, double yStep) const {
